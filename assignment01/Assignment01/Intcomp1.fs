...
let rec eval e (env : (string * int) list) : int =
    ...
    | Let(list, ebody) -> 
      match list with
      | []-> eval ebody env
      | (name, value)::xs -> eval (Let (xs, ebody)) ((name, eval value env)::env)     
...
let testExpression = Let ([("x1", CstI 4); ("x2", Prim("+", CstI 2, CstI 4))], Prim("+", Var "x1", Var "x2"))
 
let testEvaluation = eval testExpression []
